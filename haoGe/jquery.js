...
(function(global, factory) {
    ...
    jQuery.extend({
        ...
        each: function(obj, callback) {
            ...
            if (isArrayLike(obj)) { // 如果是类数组，就用普通for循环
                length = obj.length;
                for (; i < length; i++) {
        ...
        // Support: Android <=4.0 only
        trim: function(text) {
            return text == null ? // 如果无值，就返回空字符串
                "" :
                (text + "").replace(rtrim, ""); // 否则用正则，空格替换成''
        },
        ...
        map: function(elems, callback, arg) {
            ...
            if (isArrayLike(elems)) { // 分为数组和对象两种情况
                length = elems.length;
                for (; i < length; i++) {
