(function(global, factory) {
    ...
    var dir = function(elem, dir, until) { // elem 是dom元素， dir是属性， until是截至条件
        var matched = [],
            truncate = until !== undefined; // 如果没有传就是false， 传了非undefined就是true

        while ((elem = elem[dir]) && elem.nodeType !== 9) { // 每次循环都去找
            if (elem.nodeType === 1) { // 找到了元素节点
                if (truncate && jQuery(elem).is(until)) { // 如果传值了，并且
                    break;
                }
            }
        }
    ...
    var siblings = function(n, elem) {
        ...
        for (; n; n = n.nextSibling) { // 寻找下一个兄弟，循环找
